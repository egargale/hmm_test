...
def create_drawdown_chart(
    equity_curve: pd.Series, config: Optional[Dict[str, Any]] = None
) -> go.Figure:
    """
...
def create_returns_distribution_chart(
    returns: pd.Series, config: Optional[Dict[str, Any]] = None
) -> go.Figure:
    """
...
def create_monthly_returns_heatmap(
    ...
    pivot_table = monthly_returns_df.pivot_table(
        values="returns", index="month", columns="year", fill_value=0
    )
...
def create_regime_analysis_chart(
    states: np.ndarray, equity_curve: pd.Series, config: Optional[Dict[str, Any]] = None
) -> go.Figure:
    """
    ...
    if config["show_bands"]:
        for i, state_value in enumerate(unique_states):
            ...
            fig.add_vrect(
                x0=equity_curve.index[state_mask][0]
                if state_mask.any()
                else equity_curve.index[0],
                x1=equity_curve.index[state_mask][-1]
                if state_mask.any()
                else equity_curve.index[-1],
    ...
    returns_df = pd.DataFrame(
        {"returns": returns, "state": states}, index=equity_curve.index
    )

    for i, state_value in enumerate(unique_states):
        state_returns = returns_df[returns_df["state"] == state_value][
            "returns"
        ].dropna()
...
def create_performance_metrics_table(
    metrics: PerformanceMetrics, config: Optional[Dict[str, Any]] = None
) -> go.Figure:
    """
    ...
    Returns:
        Plotly figure object
    """
    default_config = {"title": "Performance Metrics Summary", "height": 400}
...
def build_dashboard(
    ...
    try:
        ...
        charts["equity_curve"] = create_equity_curve_chart(
            result.equity_curve, config=config.get("equity_curve_config")
        )

        # 2. Drawdown Chart
        charts["drawdown"] = create_drawdown_chart(
            result.equity_curve, config=config.get("drawdown_config")
        )
...
def build_html_dashboard(charts: Dict[str, go.Figure], config: Dict[str, Any]) -> str:
    ...
    html_content = html_template.format(
        title=config["title"], timestamp=timestamp, charts_html=charts_html
    )
