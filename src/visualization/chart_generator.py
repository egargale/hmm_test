...
def plot_states(
    ...
    if len(states) != len(price_data):
        raise ValueError(f"Length mismatch: states ({len(states)}) != price_data ({len(price_data)})")
    ...
    for state_value in unique_states:
        if state_value >= 0:  # Skip negative states (typically represent missing/invalid data)
            state_mask = states == state_value
    ...
    for i, indicator in enumerate(indicator_columns):
        ...
        for j, state_value in enumerate(unique_states):
            ...
            if len(state_data) > 0:
                ...
                ax.hist(state_data, bins=30, alpha=config['alpha'],
                       color=color, label=f'State {state_value}',
...
def create_regime_timeline_plot(
    ...
    ax1.plot(price_data.index, price_data.values,
             color='black', alpha=config['price_alpha'], linewidth=1)
    ...
    for i, state_value in enumerate(unique_states):
        ...
        ax2.fill_between(price_data.index, 0, 1, where=state_mask,
                         alpha=0.7, color=color, label=f'State {state_value}')
