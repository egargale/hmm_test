...
def process_streaming(
    ...
    logger.info(f"Configuration: engine={config.engine_type}, chunk_size={chunk_size}, memory_limit={memory_limit_gb}GB")
    ...
    try:
        ...
        if show_progress:
            ...
            estimated_chunks = max(1, (file_size_mb * 1024 * 1024) // (chunk_size * 1000))  # Rough estimate
            pbar = tqdm(
        ...
        for i, chunk in enumerate(chunk_reader):
            ...
            if memory_after > memory_limit_gb:
                logger.warning(f"Memory usage ({memory_after:.2f}GB) approaching limit ({memory_limit_gb}GB)")
                if memory_after > memory_limit_gb * 1.1:  # 10% buffer
                    raise MemoryError(f"Memory limit exceeded: {memory_after:.2f}GB > {memory_limit_gb}GB")
...
def _auto_detect_chunk_size(file_size_mb: float, memory_limit_gb: float) -> int:
    ...
    logger.debug(f"Auto-detected chunk size: {chunk_size} (file: {file_size_mb:.1f}MB, limit: {memory_limit_gb}GB)")
...
def monitor_memory_usage(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.debug(f"Memory after {func.__name__}: {memory_after['rss']:.2f}GB (Δ{memory_diff:+.2f}GB)")
...
def estimate_processing_time(
    ...
    try:
        ...
        if processing_times:
            ...
            estimated_chunks = int(file_size_mb * 1024 * 1024 / (chunk_size * 1000))  # Very rough
    ...
    for col in numeric_columns:
        if col in df.columns:
            ...
            if col != 'volume' and df[col].isna().any():
                ...
                logger.debug(f"Removed {na_count} NaN values from {col} column in chunk")
