...
def backtest_strategy(
    ...
    Raises:
        ValueError: If inputs are invalid
    """
    logger.info(f"Starting backtest: {len(states)} periods, {len(config.state_map)} state mappings")
    ...
    if lookahead_bias_prevention:
        ...
        logger.warning("Lookahead bias prevention is DISABLED - results may be unrealistic")
    ...
    if use_open_prices:
        ...
        logger.warning("Open price execution not fully implemented - using close prices")
        ...
        if target_position != current_position:
            ...
            if current_trade is not None:
                ...
                if current_position > 0:  # Long position
                    pnl = (current_price - current_trade.entry_price) * abs(current_trade.size)
                else:  # Short position
                    pnl = (current_trade.entry_price - current_price) * abs(current_trade.size)
                ...
                logger.debug(f"Closed trade: P&L={pnl:.2f}, duration={(current_time - current_trade.entry_time).days} days")
            ...
            if target_position != 0:
                ...
                logger.debug(f"Opened trade: {trade_action} {abs(trade_quantity)} @ {current_price}")
...
def backtest_with_analysis(
    ...
    logger.info(f"  - Position distribution: {position_analysis.get('position_distribution', {})}")
