...
class DataIntegrator:
    ...
    def __init__(self, feature_config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            "Data integrator initialized with enhanced feature engineering support"
        )

    def integrate_with_features(
        self,
        df: pd.DataFrame,
        ...
        try:
            # Standardize format
            df_standardized, format_issues = self.standardize_format(
                df, detection_result.format
            )
            ...
            performance_metrics = {
                ...
                "rows_per_second": len(df_features) / processing_time
            ...
            logger.info(
                f"Data integration completed: {len(df_features)} rows, "
    ...
    def standardize_format(
        self, df: pd.DataFrame, csv_format: CSVFormat, target_format: str = "standard"
        ...
        try:
            ...
            if csv_format.column_mapping:
                df_standardized = df_standardized.rename(
                    columns=csv_format.column_mapping
                )
            ...
            df_standardized.columns = [
                col.lower().replace(" ", "_") for col in df_standardized.columns
            ]
            ...
            if (
                "close" not in df_standardized.columns
                and "adj_close" in df_standardized.columns
            ):
            ...
            logger.debug(
                f"Format standardized: {len(df_standardized)} rows, {len(df_standardized.columns)} columns"
            )
    ...
    def _standardize_datetime(
        ...
        try:
            if csv_format.date_column and csv_format.time_column:
                ...
                if (
                    csv_format.date_column in df.columns
                    and csv_format.time_column in df.columns
                ):
                    df_datetime["datetime"] = pd.to_datetime(
                        df_datetime[csv_format.date_column]
                        + " "
                        + df_datetime[csv_format.time_column]
                    )
                    df_datetime = df_datetime.drop(
                        [csv_format.date_column, csv_format.time_column], axis=1
                    )
                ...
                if datetime_col:
                    df_datetime["datetime"] = pd.to_datetime(
                        df_datetime[datetime_col], unit="s"
                    )
    ...
    def normalize_timezones(
        self, df: pd.DataFrame, target_timezone: str = "UTC"
    ) -> Tuple[pd.DataFrame, List[str]]:
        """
        ...
        if df.index.name != "datetime" or not pd.api.types.is_datetime64_any_dtype(
            df.index
        ):
            return df, issues
    ...
    def _apply_enhanced_features(
        ...
        try:
            ...
            logger.debug(
                f"Enhanced features applied: {feature_count} new features in {processing_time:.3f}s"
            )
    ...
    def extract_metadata(self, file_path: Path, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # Date range
            if df.index.name == "datetime" and pd.api.types.is_datetime64_any_dtype(
                df.index
            ):
                metadata["date_range"] = {
    ...
    def _extract_processing_metadata(
        self,
    ...
    def _calculate_quality_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if "ohlc_consistency" in quality_metrics:
                ohlc_violation_pct = quality_metrics["ohlc_consistency"][
            ...
            if "outliers" in quality_metrics:
                ...
                outlier_percentage = (
                    total_outliers / (len(df) * len(numeric_cols))
                ) * 100
    ...
    def _generate_integration_recommendations(
        ...
        if quality_metrics.get("overall_score", 1.0) < 0.8:
            recommendations.append(
                "Data quality score is below 80%, consider data cleaning"
            )

        missing_pct = quality_metrics.get("missing_data", {}).get(
            "missing_percentage", 0
        )
        if missing_pct > 5:
            recommendations.append(
                f"High missing data percentage ({missing_pct:.1f}%), consider imputation"
            )

        # Performance-based recommendations
        if metadata.processing_time > 10:
            recommendations.append(
                "Long processing time, consider enabling performance optimizations"
            )
        ...
        if "ohlc_consistency" in quality_metrics:
            ...
            if violations > 0:
                recommendations.append(
                    f"OHLC consistency issues detected ({violations} violations)"
                )
