...
class CSVFormatDetector:
    ...
    def detect_format(
        ...
        try:
            ...
            logger.info(
                f"Format detected: {format_result.format.format_type} "
    ...
    def _detect_delimiter(self, file_path: Path, encoding: str) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Detected delimiter: '{best_delimiter}' (counts: {delimiter_counts})"
            )
    ...
    def _read_sample(
        self, file_path: Path, encoding: str, delimiter: str, sample_size: int
    ) -> pd.DataFrame:
        ...
        try:
            ...
            logger.debug(
                f"Read sample: {len(sample_df)} rows, {len(sample_df.columns)} columns"
            )
            return sample_df
    ...
    def _detect_format_type(
        self, sample_data: pd.DataFrame, delimiter: str, encoding: str
    ) -> DetectionResult:
        ...
        if patterns:
            ...
            for _expected_col, col_patterns in patterns.items():
                for actual_col in columns:
                    if any(
                        re.search(pattern, actual_col, re.IGNORECASE)
    ...
    def _create_column_mapping(
        self, actual_columns: List[str], format_config: Dict[str, Any]
    ) -> Dict[str, str]:
    ...
    def _validate_format(
        self, csv_format: CSVFormat, sample_data: pd.DataFrame
        ...
        missing_required = [
            col for col in csv_format.required_columns if col not in mapped_columns
        ]
        if missing_required:
            ...
            if col in csv_format.column_mapping:
                ...
                if standard_name in ["Open", "High", "Low", "Close", "Volume"]:
                    ...
                    try:
                        ...
                        issues.append(
                            f"Non-numeric data in {standard_name} column ({col})"
                        )
                        recommendations.append(
                            f"Clean {col} column or verify data format"
                        )
        ...
        if csv_format.date_column and csv_format.datetime_format == "ISO8601":
            if csv_format.date_column in sample_data.columns:
                try:
                    ...
                    issues.append(
                        f"Invalid datetime format in {csv_format.date_column}"
                    )
                    recommendations.append(
                        f"Verify {csv_format.date_column} contains ISO8601 dates"
                    )
    ...
    def register_custom_format(
        self, format_name: str, format_config: Dict[str, Any]
    ) -> None:
