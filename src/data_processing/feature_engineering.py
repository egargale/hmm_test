...
def add_features(
    ...
    # 2. Moving averages
    df = _add_moving_averages(
        df, indicator_config.get("moving_averages", {}), min_periods
    )
    features_added.extend([col for col in df.columns if "ma_" in col or "ema_" in col])

    # 3. Volatility indicators
    df = _add_volatility_indicators(
        df, indicator_config.get("volatility", {}), min_periods
    )
    features_added.extend(
        [col for col in df.columns if "atr" in col or "volatility" in col]
    )
    ...
    features_added.extend(
        [
            ...
            if any(x in col for x in ["rsi", "roc", "macd", "stoch"])
        ]
    )
    ...
    features_added.extend(
        [col for col in df.columns if any(x in col for x in ["volume_", "obv", "vwap"])]
    )

    # 6. Trend indicators
    df = _add_trend_indicators(df, indicator_config.get("trend", {}), min_periods)
    features_added.extend(
        [col for col in df.columns if any(x in col for x in ["adx", "cci", "dpo"])]
    )

    # 7. Price patterns
    df = _add_price_patterns(df, indicator_config.get("patterns", {}), min_periods)
    features_added.extend(
        [col for col in df.columns if any(x in col for x in ["bb_", "kc_", "dc_"])]
    )

    # 8. Enhanced momentum indicators
    df = _add_enhanced_momentum_indicators(
        df, indicator_config.get("enhanced_momentum", {}), min_periods
    )
    enhanced_momentum_features = [
        col
        for col in df.columns
        if any(x in col for x in ["williams_r", "cci", "mfi", "mtm", "proc"])
    ]
    features_added.extend(enhanced_momentum_features)

    # 9. Enhanced volatility indicators
    df = _add_enhanced_volatility_indicators(
        df, indicator_config.get("enhanced_volatility", {}), min_periods
    )
    enhanced_volatility_features = [
        col
        for col in df.columns
        if any(x in col for x in ["chaikin_vol", "hv_", "keltner_", "donchian_"])
    ]
    features_added.extend(enhanced_volatility_features)

    # 10. Enhanced trend indicators
    df = _add_enhanced_trend_indicators(
        df, indicator_config.get("enhanced_trend", {}), min_periods
    )
    enhanced_trend_features = [
        col
        for col in df.columns
        if any(x in col for x in ["tma_", "wma_", "hma_", "aroon_", "di_", "adx_"])
    ]
    features_added.extend(enhanced_trend_features)

    # 11. Enhanced volume indicators
    df = _add_enhanced_volume_indicators(
        df, indicator_config.get("enhanced_volume", {}), min_periods
    )
    enhanced_volume_features = [
        col
        for col in df.columns
        if any(x in col for x in ["adl", "vpt", "eom_", "volume_roc_"])
    ]
    ...
    time_features = [
        ...
        for col in df.columns
        if any(
            x in col for x in ["day_of_", "month", "quarter", "hour", "session", "is_"]
        )
    ]
    ...
    custom_features = [
        col
        for col in df.columns
        if col not in df.columns[: len(df.columns) - len(features_added)]
    ]
...
def _add_moving_averages(
    df: pd.DataFrame, config: Dict[str, Any], min_periods: int
) -> pd.DataFrame:
...
def _add_volatility_indicators(
    ...
    if "length" in atr_config:
        ...
        atr = AverageTrueRange(
            df["high"], df["low"], df["close"], window=length, fillna=False
        )
    ...
    if "length" in bbands_config:
        ...
        df[f"bb_position_{length}"] = (
            df["close"] - bbands.bollinger_lband()
...
def _add_volume_indicators(
    ...
    if "length" in volume_sma_config:
        ...
        df[f"volume_sma_{length}"] = (
            df["volume"].rolling(window=length, min_periods=min_periods).mean()
        )
...
def _add_enhanced_momentum_indicators(
    df: pd.DataFrame, config: Dict[str, Any], min_periods: int
) -> pd.DataFrame:
    ...
    if "length" in williams_config:
        ...
        williams = WilliamsRIndicator(
            df["high"], df["low"], df["close"], lbp=length, fillna=False
        )
    ...
    if "length" in cci_config:
        ...
        mean_deviation = typical_price.rolling(
            window=length, min_periods=min_periods
    ...
    if "length" in mfi_config:
        ...
        positive_mf_sum = (
            pd.Series(positive_mf).rolling(window=length, min_periods=min_periods).sum()
        )
        negative_mf_sum = (
            pd.Series(negative_mf).rolling(window=length, min_periods=min_periods).sum()
        )
...
def _add_enhanced_volatility_indicators(
    df: pd.DataFrame, config: Dict[str, Any], min_periods: int
) -> pd.DataFrame:
    ...
    if "ema_period" in chaikin_config and "roc_period" in chaikin_config:
        ...
        chaikin_vol = (
            (ema_hl - ema_hl.shift(roc_period)) / ema_hl.shift(roc_period)
        ) * 100
    ...
    if "window" in hv_config:
        ...
        historical_vol = log_returns.rolling(
    ...
    if all(k in keltner_config for k in ["ema_period", "atr_period", "atr_multiplier"]):
        ...
        # Keltner Channels
        df[f"keltner_upper_{ema_period}_{atr_period}"] = ema_close + (
            atr_multiplier * atr
        )
        df[f"keltner_lower_{ema_period}_{atr_period}"] = ema_close - (
            atr_multiplier * atr
        )
    ...
    if "period" in donchian_config:
        ...
        df[f"donchian_upper_{period}"] = (
            df["high"].rolling(window=period, min_periods=min_periods).max()
        )
        df[f"donchian_lower_{period}"] = (
            df["low"].rolling(window=period, min_periods=min_periods).min()
        )
...
def _add_enhanced_trend_indicators(
    df: pd.DataFrame, config: Dict[str, Any], min_periods: int
) -> pd.DataFrame:
    ...
    if "period" in tma_config:
        ...
        df[f"tma_{period}"] = sma1.rolling(
            window=half_period, min_periods=min_periods
        ).mean()
    ...
    if "period" in hma_config:
        ...
        wma_half = (
            df["close"]
            .rolling(window=half_period, min_periods=min_periods)
            .apply(
                lambda x: np.dot(x, np.arange(1, half_period + 1))
        ...
        hma_series = (
            (2 * wma_half - wma_full)
            .rolling(window=sqrt_period, min_periods=min_periods)
            .apply(
    ...
    if "period" in aroon_config:
        ...
        aroon_up = (
            df["high"]
            .rolling(window=period, min_periods=min_periods)
            .apply(lambda x: (period - 1 - x.argmax()) / (period - 1) * 100, raw=True)
        )
        ...
        aroon_down = (
            df["low"]
            .rolling(window=period, min_periods=min_periods)
            .apply(lambda x: (period - 1 - x.argmin()) / (period - 1) * 100, raw=True)
        )
    ...
    if "period" in dmi_config:
        ...
        di_plus = (
            pd.Series(dm_plus).rolling(window=period, min_periods=min_periods).mean()
            / atr
            * 100
        )
        di_minus = (
            pd.Series(dm_minus).rolling(window=period, min_periods=min_periods).mean()
            / atr
            * 100
        )
...
def _add_enhanced_volume_indicators(
    df: pd.DataFrame, config: Dict[str, Any], min_periods: int
) -> pd.DataFrame:
    ...
    if "period" in volume_roc_config:
        ...
        df[f"volume_roc_{period}"] = (
            (df["volume"] - df["volume"].shift(period)) / df["volume"].shift(period)
        ) * 100
...
class FeatureEngineer:
    ...
    def _create_indicator_config(self) -> Dict[str, Dict[str, Any]]:
        ...
        if self.config.enable_sma_ratios:
            config["moving_averages"] = {
                "sma_ratio": {
                    "fast": self.config.sma_window // 2,
                    "slow": self.config.sma_window,
                }
            }
        ...
        if (
            hasattr(self.config, "enable_historical_volatility")
            and self.config.enable_historical_volatility
        ):
            config.setdefault("enhanced_volatility", {})["historical_volatility"] = {
                "window": 20
            }
        if (
            hasattr(self.config, "enable_keltner_channels")
            and self.config.enable_keltner_channels
        ):
        ...
        # Time-based features
        if (
            hasattr(self.config, "enable_time_features")
            and self.config.enable_time_features
        ):
    ...
    def get_feature_names(self) -> List[str]:
        ...
        if self._feature_names is None:
            raise ValueError(
                "Features have not been computed yet. Call add_features() first."
            )
    ...
    def get_feature_importance(
        self, df: pd.DataFrame, target_col: str = "log_ret"
    ) -> pd.DataFrame:
        """
    ...
    def apply_feature_selection(
        ...
        **selection_params,
        ...
        logger.info(
            f"Feature selection: {len(X.columns)} -> {len(X_selected.columns)} features"
        )
        return X_selected
    ...
    def assess_feature_quality(
        ...
        logger.info(
            f"Quality filtering: {len(X.columns)} -> {len(filtered_X.columns)} features"
        )
    ...
    def get_enhanced_feature_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for feature in self._feature_names:
            if any(x in feature for x in ["log_ret", "simple_ret"]):
                ...
                x in feature for x in ["williams_r", "cci_", "mfi_", "mtm_", "proc_"]
            ):
                feature_categories["enhanced_momentum"].append(feature)
            elif any(
                x in feature for x in ["chaikin_vol", "hv_", "keltner_", "donchian_"]
            ):
                ...
                x in feature
                for x in ["day_of_", "month", "quarter", "hour", "session", "is_"]
            ):
