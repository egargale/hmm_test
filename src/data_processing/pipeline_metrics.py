...
class PipelineMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_stage_time": sum(sm.processing_time for sm in self.stage_metrics)
...
class MetricsCollector:
    ...
    def start_stage(
        self, stage_name: str, input_metadata: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    def end_stage(
        ...
        if output_metadata:
            ...
            self.current_stage.memory_usage_mb = output_metadata.get(
                "memory_usage_mb", 0
            )
        ...
        self.logger.debug(
            f"Ended metrics collection for stage: {self.current_stage.stage_name}"
        )
    ...
    def _calculate_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stage_efficiency": len(successful_stages)
            / len(self.metrics.stage_metrics),
            "bottleneck_stage": max(
                self.metrics.stage_metrics, key=lambda sm: sm.processing_time
            ).stage_name
            if self.metrics.stage_metrics
            else None,
        }
    ...
    def _calculate_resource_usage(self) -> Dict[str, Any]:
        ...
        memory_usage = [
            sm.memory_usage_mb
            for sm in self.metrics.stage_metrics
            if sm.memory_usage_mb > 0
        ]
        ...
        return {
            ...
            "memory_efficiency": max(memory_usage) / sum(memory_usage)
            if sum(memory_usage) > 0
            else 0,
        }
...
class PerformanceProfiler:
    ...
    def get_profile_summary(self, profile_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_cpu_usage": sum(s["cpu_usage_avg"] for s in sessions)
            / len(sessions),
        }
...
class MetricsReporter:
    ...
    def generate_performance_report(self, metrics: PipelineMetrics) -> Dict[str, Any]:
        ...
        if metrics.stage_metrics:
            ...
            report["performance_analysis"] = {
                ...
                "bottleneck_stage": max(
                    metrics.stage_metrics, key=lambda sm: sm.processing_time
                ).stage_name,
    ...
    def generate_quality_report(self, metrics: PipelineMetrics) -> Dict[str, Any]:
        ...
        if metrics.quality_metrics:
            report["quality_breakdown"] = {
                "completeness_score": metrics.quality_metrics.get(
                    "completeness_score", 0
                ),
                "validation_score": metrics.quality_metrics.get("validation_score", 0),
                "feature_quality_score": metrics.quality_metrics.get(
        ...
        report["issues_summary"] = {
            ...
            "stages_with_issues": len(
                [sm for sm in metrics.stage_metrics if sm.issues]
            ),
            "issue_categories": self._categorize_issues(metrics),
        }

        # Improvement suggestions
        report["improvement_suggestions"] = self._generate_quality_recommendations(
            metrics
        )
        ...
        return {
            ...
            "pipeline_info": {
                ...
                "stages_successful": sum(
                    1 for sm in metrics.stage_metrics if sm.success
                ),
            },
            "data_info": {
                "input_shape": (
                    metrics.input_metadata.get("rows", 0),
                    metrics.input_metadata.get("columns", 0),
                ),
                "output_shape": (
                    metrics.output_metadata.get("rows", 0),
                    metrics.output_metadata.get("columns", 0),
                ),
                "features_added": metrics.output_metadata.get("columns", 0)
                - metrics.input_metadata.get("columns", 0),
            },
            "performance_highlights": {
                "total_processing_time": metrics.total_processing_time,
                "average_stage_time": sum(
                    sm.processing_time for sm in metrics.stage_metrics
                )
    ...
    def _calculate_efficiency_score(self, metrics: PipelineMetrics) -> float:
        ...
        success_rate = sum(1 for sm in metrics.stage_metrics if sm.success) / len(
            metrics.stage_metrics
        )

        # Time efficiency (inverse of total time)
        time_efficiency = 1.0 / (
            1.0 + metrics.total_processing_time / 60
        )  # Normalize to minutes

        # Memory efficiency
        memory_usage = [
            sm.memory_usage_mb for sm in metrics.stage_metrics if sm.memory_usage_mb > 0
        ]
        memory_efficiency = (
            1.0 / (1.0 + (max(memory_usage) / 1000)) if memory_usage else 1.0
        )  # Normalize to GB
    ...
    def _generate_performance_recommendations(
        ...
        stage_times = [
            (sm.stage_name, sm.processing_time) for sm in metrics.stage_metrics
        ]
        ...
        for stage_name, time_taken in stage_times:
            if time_taken > avg_time * 2:
                recommendations.append(
                    f"Consider optimizing '{stage_name}' stage ({time_taken:.2f}s vs avg {avg_time:.2f}s)"
                )
        ...
        failed_stages = [
            sm.stage_name for sm in metrics.stage_metrics if not sm.success
        ]
        if failed_stages:
            recommendations.append(
                f"Address issues in failed stages: {', '.join(failed_stages)}"
            )
    ...
    def _generate_quality_recommendations(self, metrics: PipelineMetrics) -> List[str]:
        ...
        if overall_score < 0.7:
            recommendations.append(
                "Overall data quality is below 70% - review validation issues"
            )
        ...
        if feature_quality < 0.8:
            recommendations.append(
                "Consider feature selection to improve feature quality"
            )
        ...
        if total_issues > 10:
            recommendations.append(
                "High number of issues detected - review data preprocessing"
            )
    ...
    def _categorize_issues(self, metrics: PipelineMetrics) -> Dict[str, int]:
        ...
        categories = {"validation": 0, "processing": 0, "memory": 0, "other": 0}
        ...
        for stage_metric in metrics.stage_metrics:
            for issue in stage_metric.issues:
                ...
                if any(
                    ...
                    for keyword in ["validation", "invalid", "missing"]
                ):
                    categories["validation"] += 1
                elif any(
                    keyword in issue_lower
                    for keyword in ["processing", "failed", "error"]
                ):
