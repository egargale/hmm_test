...
def validate_inference_inputs(
    model: hmm.GaussianHMM, scaler: StandardScaler, features: np.ndarray
) -> None:
    """
    ...
    if features.shape[1] != model.n_features:
        raise ValueError(
            f"Feature dimension mismatch: expected {model.n_features}, got {features.shape[1]}"
        )

    if scaler.n_features_in_ != features.shape[1]:
        raise ValueError(
            f"Scaler dimension mismatch: expected {scaler.n_features_in_}, got {features.shape[1]}"
        )
...
def predict_states(
    ...
    Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray, float], np.ndarray
]:
    """
    ...
    try:
        ...
        if return_probabilities:
            ...
            if not np.allclose(np.sum(probabilities, axis=1), 1.0, rtol=1e-5):
                logger.warning(
                    "Posterior probabilities do not sum to 1 (possible numerical issues)"
                )
...
def predict_states_comprehensive(
    ...
    if n_samples > 1:
        for i in range(n_samples - 1):
            ...
            expected_transitions[transition] = (
                expected_transitions.get(transition, 0) + 1
            )
    ...
    metadata = {
        ...
        "state_percentages": {
            k: v / n_samples * 100 for k, v in state_distribution.items()
        },
        ...
        "expected_transitions_per_sample": sum(expected_transitions.values())
        / max(n_samples - 1, 1),
    ...
    logger.info(
        f"Comprehensive inference completed: {n_samples} samples, {n_states} states"
    )
...
def get_lagged_states(
    states: np.ndarray, lag_periods: int = 1, fill_value: int = -1
) -> np.ndarray:
    """
...
def predict_states_with_lag(
    ...
    logger.info(
        f"Lagged inference completed: {metadata['valid_samples']}/{len(states)} valid predictions"
    )
...
def analyze_state_stability(
    ...
    states = predict_states(
        model, scaler, features, return_probabilities=False, return_log_likelihood=False
    )
    ...
    transition_probs = transition_counts / np.sum(
    ...
    logger.info(
        f"State stability analysis completed: change_rate={analysis['change_rate']:.3f}, "
...
def create_inference_dataframe(
    ...
    if lag_periods > 0:
        ...
        comprehensive_result = predict_states_comprehensive(
            model, scaler, features, feature_names
        )
    ...
    if timestamps is not None:
        if len(timestamps) != n_samples:
            raise ValueError(
                f"Timestamps length {len(timestamps)} doesn't match samples {n_samples}"
            )
        index = timestamps
    else:
    ...
    logger.info(
        f"Created inference DataFrame with {len(df)} rows and {len(df.columns)} columns"
    )
