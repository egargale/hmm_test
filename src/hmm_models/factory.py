...
class HMMModelFactory:
    ...
    def create_model(
        ...
        **kwargs
        ...
        model_config = self._validate_config(model_type, model_config, n_samples, n_features)
    ...
    def create_model_ensemble(
        ...
        for model_type in model_types:
            for n_components in n_components_range:
                ...
                config_combinations = self._generate_config_combinations(base_config, config_variations)
                ...
                for config in config_combinations:
                    try:
                        ...
                        logger.warning(f"Failed to create {model_type} model with config {config}: {e}")
    ...
    def auto_select_model_type(
        ...
        logger.info(f"Auto-selecting model for {n_samples} samples, {n_features} features")
        logger.info(f"Data complexity: {data_complexity}, Budget: {computational_budget}")
    ...
    def _validate_config(
        ...
        if n_samples is not None and 'n_components' in validated_config:
            max_components = min(n_samples // 10, 10)  # Rule of thumb: at least 10 samples per state
            if validated_config['n_components'] > max_components:
                logger.warning(f"Reducing n_components from {validated_config['n_components']} to {max_components} "
        ...
        if model_type == 'gmm' and 'n_mix' in validated_config:
            if n_samples is not None:
                ...
                if validated_config['n_mix'] > max_mix:
                    logger.warning(f"Reducing n_mix from {validated_config['n_mix']} to {max_mix} "
        ...
        if 'covariance_type' in validated_config:
            ...
            if validated_config['covariance_type'] not in valid_types:
                logger.warning(f"Invalid covariance_type {validated_config['covariance_type']}, using 'full'")
        ...
        if 'n_iter' in validated_config and n_samples is not None:
            if n_samples > 50000:
                ...
                logger.info(f"Reduced n_iter to {validated_config['n_iter']} for large dataset")
    ...
    def _get_model_capabilities(self, model_type: str) -> Dict[str, Any]:
        ...
        capabilities = {
            'gaussian': {
                ...
                'best_for': ['simple_regimes', 'unimodal_distributions', 'limited_data'],
            ...
            'gmm': {
                ...
                'best_for': ['complex_regimes', 'multi_modal_distributions', 'market_microstructure'],
                'limitations': ['Higher computational cost', 'More parameters to estimate']
            }
        }
