...
class HMMConfig(BaseModel):
    ...
    covariance_type: str = Field("diag", description="Covariance type ('full', 'diag', 'tied', 'spherical')")
...
class ProcessingConfig(BaseModel):
    ...
    engine_type: str = Field("streaming", description="Processing engine ('streaming', 'dask', 'daft')")
    chunk_size: int = Field(10000, ge=1, description="Chunk size for streaming processing")
    ...
    downcast_floats: bool = Field(True, description="Downcast float64 to float32 for memory efficiency")
...
class BacktestConfig(BaseModel):
    ...
    lookahead_lag: int = Field(1, ge=0, description="Number of periods to lag for lookahead bias prevention")
    ...
    def validate_state_map(cls, v: Dict[int, int]) -> Dict[int, int]:
        ...
        for state, position in v.items():
            ...
            if position not in [-1, 0, 1]:
                raise ValueError(f"Position {position} for state {state} must be -1, 0, or 1")
        return v
...
def load_config(config_path: Union[str, Path]) -> Config:
    ...
    try:
        ...
        raise json.JSONDecodeError(f"Error parsing JSON config file {config_path}: {e.msg}, line {e.lineno}")
...
def save_config(config: Config, config_path: Union[str, Path]) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"Error saving config file {config_path}: {e}")
