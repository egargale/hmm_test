...
def _make_contaminated_data(n_per_state=200, n_features=2, n_outliers=10, seed=42):
    ...
    means_true = np.array([[-5.0] * n_features, [0.0] * n_features, [5.0] * n_features])
...
class TestRegistry:
    ...
    def test_registry_resolves_to_robust_engine(self):
        ...
        assert ENGINE_REGISTRY["robust_hmm"][0] is RobustHMMEngine
...
class TestHuberRobustness:
    ...
    def test_huber_correction_reduces_outlier_bias(self):
        ...
        n_per_state = 300
        means_true = np.array(
            [[-6.0] * n_features, [0.0] * n_features, [6.0] * n_features]
        )
