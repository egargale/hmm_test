...
def test_enhanced_indicators():
    ...
    enhanced_indicators = [
        ...
        'hv_20', 'keltner_upper_20_10', 'donchian_upper_20',
        'tma_20', 'wma_10', 'aroon_up_14', 'di_plus_14',
        'adl', 'vpt', 'eom_14', 'volume_roc_10',
        'day_of_week', 'hour', 'session', 'day_of_week_sin', 'hour_cos'
    ]

    found_indicators = [ind for ind in enhanced_indicators if ind in df_with_features.columns]
    print(f"✓ Enhanced indicators calculated: {len(found_indicators)}/{len(enhanced_indicators)}")
...
def test_feature_selection():
    ...
    print(f"✓ Correlation selection: {len(X.columns)} -> {len(features_corr.columns)} features")
    ...
    print(f"✓ Variance selection: {len(X.columns)} -> {len(features_var.columns)} features")
    ...
    print(f"✓ Quality filtering: {len(X.columns)} -> {len(filtered_features.columns)} features")
...
def test_performance_benchmarks():
    ...
    if len(numeric_features) > 20:
        ...
        if len(features) > 20:
            ...
            print(f"  - Features reduced: {len(X.columns)} -> {len(selected_features.columns)}")
            print(f"  - Selection speed: {len(X.columns) / selection_time:.0f} features/second")
