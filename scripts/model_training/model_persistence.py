...
def generate_model_hash(model: hmm.GaussianHMM, scaler: StandardScaler) -> str:
    ...
    hash_data = {
        ...
        "model_covars": [cov.tolist() for cov in model.covars_]
        if hasattr(model, "covars_")
        else None,
        "model_startprob": model.startprob_.tolist()
        if hasattr(model, "startprob_")
        else None,
        "model_transmat": model.transmat_.tolist()
...
def save_model(
    ...
    if path.exists() and not overwrite:
        raise FileExistsError(
            f"Model file already exists at {path}. Use overwrite=True to replace."
        )
...
def load_model(
    ...
    try:
        ...
        logger.info(
            f"Model loaded successfully: {model.n_components} states, {model.n_features} features"
        )
...
def _validate_model_integrity(model_data: Dict[str, Any], path: Path) -> None:
    ...
    if missing_keys:
        raise ValueError(
            f"Corrupted model file {path}: missing required keys {missing_keys}"
        )
    ...
    if saved_version != MODEL_VERSION:
        logger.warning(
            f"Model version mismatch: expected {MODEL_VERSION}, got {saved_version}"
        )
...
def _validate_loaded_components(
    model: Any, scaler: Any, config_dict: Dict[str, Any], path: Path
) -> None:
    """
    ...
    if missing_config_keys:
        raise ValueError(
            f"Invalid config in {path}: missing keys {missing_config_keys}"
        )
    ...
    try:
        ...
        if test_features is None:
            ...
            if test_features.shape[1] != model.n_features:
                raise ValueError(
                    f"Test features dimension mismatch: "
        ...
        if len(states) != len(test_features):
            raise ValueError(
                f"Model prediction failed: expected {len(test_features)} states, "
                f"got {len(states)}"
            )
        ...
        if probabilities.shape != (len(test_features), model.n_components):
            raise ValueError(
                f"Model probability computation failed: "
        ...
        raise ValueError(
            f"Model functionality validation failed for {path}: {e}"
        ) from e
...
def list_saved_models(directory: Union[str, Path]) -> Dict[str, Dict[str, Any]]:
    ...
    for file_path in directory.rglob("*.pkl"):
        try:
            ...
            models[str(file_path)] = {"filename": file_path.name, "error": str(e)}
