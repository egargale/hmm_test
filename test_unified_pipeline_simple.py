...
def create_test_data():
    ...
    for i in range(n_samples):
        ...
        close_prices[i] = (
            low_prices[i] + (high_prices[i] - low_prices[i]) * close_position
        )
...
def test_basic_pipeline():
    ...
    try:
        ...
        assert len(result.data.columns) >= len(test_data.columns), (
            "Should have at least original columns"
        )

        print(
            f"  ✓ Pipeline processed {len(result.data)} rows with {len(result.data.columns)} columns"
        )
        print(
            f"  ✓ Features added: {len(result.data.columns) - len(test_data.columns)}"
        )
    ...
    try:
        ...
        try:
            ...
            config_csv = PipelineConfig(
                input_type=InputSourceType.CSV_FILE, input_source=csv_file
            )
...
def test_feature_integration():
    ...
    try:
        ...
        print(
            f"  ✓ Column change: {features_added} (expected 0 with features disabled)"
        )
...
def test_performance_characteristics():
    ...
    try:
        ...
        for size in sizes:
            ...
            rows_per_second = (
                len(result.data) / processing_time
                if result.success and processing_time > 0
                else 0
            )
            ...
            print(
                f"    ✓ {size} rows: {processing_time:.3f}s, {rows_per_second:.0f} rows/sec"
            )

        # Validate performance
        avg_speed = sum(
            r["rows_per_second"] for r in results.values() if r["success"]
        ) / len(results)
