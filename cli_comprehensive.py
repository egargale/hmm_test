            ...
            with open(config_file) as f:
                if config_file.endswith('.json'):
                    ...
                    if YAML_AVAILABLE:
                        ...
                        raise click.ClickException("YAML support requires PyYAML package")
...
@click.option('--max-iter',
              type=int,
              default=100,
              help='Maximum HMM iterations (default: 100)')
@click.option('--num-restarts',
              type=int,
              default=3,
...
@click.option('--generate-charts/--no-generate-charts',
              default=False,
              help='Generate visualization charts')
@click.option('--generate-dashboard/--no-generate-dashboard',
              default=False,
              help='Generate interactive dashboard')
@click.option('--generate-report/--no-generate-report',
              default=False,
              help='Generate detailed report')
@click.pass_context
def analyze(ctx, input_csv, output_dir, engine, chunk_size, n_states, test_size,
             random_seed, covariance_type, max_iter, num_restarts, model_out,
    ...
    try:
        ...
        try:
            ...
            log_performance_metrics(
                step_start_time, "results_saving",
                {
        ...
        # Performance summary
        log_performance_metrics(
            pipeline_start_time, "complete_pipeline",
            {
...
def _generate_charts(results, output_dir, logger):
    ...
    try:
        ...
        ax2.pie(state_counts.values, labels=[f'State {s}' for s in state_counts.index],
                autopct='%1.1f%%')
...
def model_info(ctx, model_path):
    ...
    try:
        ...
        raise click.ClickException(f"Failed to load model information: {e}")
